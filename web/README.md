...
The assets tNVDAx and twNVDAx are deployment test tokens, not issuer-backed stocks. USDG amounts in this deployment use Payoff Test USDG (`tUSDG`), a project-owned token with 6 decimals. The **Get test tokens** page (`/faucet`) lets any connected wallet mint an arbitrary positive amount repeatedly, with no quota or cooldown; only test OKB gas is needed. It sends a direct transaction to `TestnetUSDG.faucet(amount)` and uses the normal transaction receipt/recovery flow. These freely mintable tokens have no monetary value. **My positions** recovers positions from Exchange events and Vault state, including series no longer in the active catalog. Eligible positions can be claimed after dealer exercise or expiry. Clearing browser storage does not remove onchain positions. An RPC error clears unavailable data and never falls back to a simulated balance or quote.
...
- New testers need test OKB for gas, can obtain tUSDG from `/faucet`, and still need owner-minted test stock for Sell High. There is no integrated stock faucet or wrapped-stock redemption UI; claims deliver the contract-specified token, including wrapped stock.
